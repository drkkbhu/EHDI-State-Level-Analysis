...
figure_labels <- c(
  "Dr" = "Diagnostic rate",
  "PerTscr" = "Infants receiving hearing screening (%)",
  "Per_rep_women_uninsured" = "Reproductive-age women uninsured (%)",
  "Per_infant_breastfed" = "Infants ever breastfed (%)",
  "state_per_capita_fund" = "State public health funding per capita",
...
res_combined <- bind_rows(
  ...
  mutate(
    ...
    term_label = ifelse(term %in% names(figure_labels), figure_labels[term], term),
...
res_combined <- res_combined %>%
  mutate(
    ...
    p_lab   = sprintf("p = %.2f", p.value),

    # tag only the rows you want to label
    label_flag = !is.na(p.value) & p.value < 0.05
  )
...
# -----------------------------
  ...
  scale_color_manual(values = c("Pooled OLS" = "#2c3e50", 
                                ...
                                "Between" = "#3498db"),
                     labels =c(
                       "Pooled OLS" = "Pooled OLS model",
                       "Two-Way FE" = "Two-way FE model",
                       "Between"= "Between-state model"
                     )) +
  labs(
    title = "Associations of state-level characteristics with EHDI outcomes",
    subtitle = "Estimates from pooled OLS, two-way fixed-effects, and between-state models",
    x = "Standardized regression coefficient",
    y = "State-level predictors",
    color = "Model Type"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "bottom",
    legend.title = element_text(face="plain",hjust = 0.5,margin = margin(b = 4)),
    # Bold and Black Y-axis labels (Variable Names)
    axis.text.y = element_text(color = "black"),
    # Bold and Black X-axis labels (Estimates)
    axis.text.x = element_text(color = "black"),
    # Bold Axis Title
    axis.title.x = element_text(face = "plain", color = "black", margin = margin(t = 10)),
    # Format the panel headers (HLp and EIr)
    strip.text = element_text(face = "bold", size = 12, color = "black"),
    panel.spacing = unit(2, "lines"),
    plot.title = element_text(face = "bold", size = 16, hjust = -0.5,margin = margin(b = 4)),
    plot.subtitle = element_text(face="plain",size = 12, hjust = 0.2,margin = margin(b = 4)),
    panel.grid.minor = element_blank()
  )
# save plot
#ggsave("Supplementary_Figure2_Forest_plot_revision2.1.TIFF", width = 10, height = 6.74, dpi = 300, compression = "lzw")
```
...
label(ehdi_pos$HLp) <- "Hearing Loss Prevalence (HLp)"
label(ehdi_pos$EIr) <- "Early Intervention Rate (EIr)"
label(ehdi_pos$PerTscr) <- "Infants receiving hearing screening (%)"
label(ehdi_pos$Dr) <- "Diagnostic Rate"
label(ehdi_pos$Median_household_income) <- "Median Household Income"
label(ehdi_pos$state_per_capita_fund) <- "State public health funding per capita"
label(ehdi_pos$Per_rep_women_uninsured) <- "Reproductive-age women uninsured (%)"
label(ehdi_pos$Per_infant_breastfed) <- "Infants ever breastfed (%)"
...
ggplot(
  ...
  facet_grid(. ~ factor(Outcome, levels=c("HLp", "EIr"), labels=c("HLp" = "Hearing loss prevalence (HLp)", "EIr" = "Early intervention rate (EIr)")), scales = "free_x",) +
   scale_color_manual(
        values = c(
            "HLp" = "#2c3e50",
            "EIr" = "#3498db"
  ...
  labs(
    title = "Breastfeeding as a Modifier of Associations With EHDI Outcomes",
    subtitle = "Interaction coefficients from two-way fixed-effects models",
    x = "Interaction coefficient (95% CI)",
    y = "Breastfeeding interaction terms",
    color = "Outcome"
  ) +
  ...
  theme(
    ...
    strip.text = element_text(face = "bold", size = 12, color = "black"),
    panel.spacing = unit(2, "lines"),
    plot.title = element_text(face = "bold", size = 16, hjust = 1.2,margin = margin(b = 4)),
    plot.subtitle = element_text(face="plain",size = 12, hjust = -0.2,margin = margin(b = 10)),
    panel.grid.minor = element_blank()
  ) 

# For Journals TIFF image
#ggsave("Supplementary_Figure3_Interaction_plot_review2.tiff", width = 10, height = 5, dpi = 300, compression = "lzw")
